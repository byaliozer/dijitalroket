            ...
            >
              {it.cover_image && (
                <div className="relative h-48 overflow-hidden bg-[#F8FAFC]">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-[#07111F]/40 to-transparent" />
                  <div className="absolute bottom-3 left-3 flex flex-wrap gap-1.5">
                    {(it.tags || []).slice(0, 3).map((t) => (
                      <span key={t} className="rounded-md bg-white/90 backdrop-blur px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-[#07111F]">{t}</span>
                    ))}
                  </div>
                </div>
              )}
              <div className="p-7 flex-1 flex flex-col">
                <div className="text-xs font-bold uppercase tracking-[0.18em] text-[#2563EB]">{it.sector}</div>
                <h3 className="mt-2 h3-card">{it.title}</h3>
                <dl className="mt-5 space-y-3 text-sm">
                  <div>
                    <dt className="font-semibold text-[#07111F]">İhtiyaç</dt>
                    <dd className="text-[#334155] mt-0.5">{it.need}</dd>
                  </div>
                  <div>
                    <dt className="font-semibold text-[#07111F]">Çözüm</dt>
                    <dd className="text-[#334155] mt-0.5">{it.solution}</dd>
                  </div>
                  <div>
                    <dt className="font-semibold text-[#07111F]">Sonuç</dt>

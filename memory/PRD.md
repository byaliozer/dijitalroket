...
- **(2026-06-11) Logo freedom + contact-info overlays**: Removed the strict logo prompt and the 9-point logo position selector entirely (admin + portal + backend). AI now places & sizes the logo freely/organically (still must NOT crop or recolor it). Added brand `instagram` and `phone` fields (website = brand_url). Generate panel shows per-field checkboxes (`contact-website/instagram/phone`) — only the checked contact details are rendered into the image (`include_website/instagram/phone` flags → `_contact_block` prompt). Settings modal + admin form updated accordingly. VALIDATED live: a first-aid poster generated with logo naturally integrated + website/Instagram/phone correctly placed as text.
